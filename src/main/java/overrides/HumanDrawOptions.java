...
public class HumanDrawOptions {
	...
	public static necesse.gfx.drawOptions.human.HumanDrawOptions fromCustom(Level level, HumanDrawOptions options) {
	    // Create a new instance of HumanDrawOptions from necesse
	    necesse.gfx.drawOptions.human.HumanDrawOptions newOptions = new necesse.gfx.drawOptions.human.HumanDrawOptions(level);

	    try {
	        // Get all fields of the HumanDrawOptions class (including private ones)
	        Field[] fields = HumanDrawOptions.class.getDeclaredFields();

	        for (Field field : fields) {
	            field.setAccessible(true);  // Make private fields accessible

	            // Clone or copy field members dynamically based on field type
	            if (field.getType() == RaceLook.class) {
	                field.set(newOptions, field.get(options)); // Directly copy reference if immutable or safe
	            } else if (field.getType() == List.class) {
	                // Clone List fields
	                if (field.getName().equals("behindOptions")) {
	                    field.set(newOptions, new ArrayList<>((List<?>) field.get(options)));
	                } else if (field.getName().equals("topOptions")) {
	                    field.set(newOptions, new ArrayList<>((List<?>) field.get(options)));
	                } else if (field.getName().equals("onBodyOptions")) {
	                    field.set(newOptions, new ArrayList<>((List<?>) field.get(options)));
	                }
	            } else {
	                // For other fields, directly copy the value
	                field.set(newOptions, field.get(options));
	            }
	        }
	...
	public HumanDrawOptions(Level level, RaceLook look, boolean onlyHumanlike) {
		this(level);
		this.look = look;
	...
	public HumanDrawOptions hatTexture(GameTexture hatTexture, ArmorItem.HairDrawMode mode, int xOffset, int yOffset) {
	    return this.hatTexture(
	        (HumanDrawOptionsGetter) (player, dir, spriteX, spriteY, spriteRes, drawX, drawY, width, height, mirrorX, mirrorY, light, alpha, mask) -> 
	            hatTexture.initDraw()
	...
	public HumanDrawOptions attackOffsets(int centerX, int centerY, int armRotateX, int armRotateY, int armLength,
	...
	public HumanDrawOptions attackOffsets(int centerX, int centerY, int armLength, int armCenterHeight,
			int itemYOffset) {
		this.attackCenterX = centerX;
		this.attackCenterY = centerY;
		this.attackArmLength = armLength;
	...
	public HumanDrawOptions itemAttack(InventoryItem item, PlayerMob player, float attackProgress, float attackDirX,
			float attackDirY) {
	...
	public DrawOptions pos(int drawX, int drawY) {
		...
		if (this.hatTexture != null) {
			...
			// literally the only item that has this is a single cosmetic.
		...
		Iterator var51 = this.behindOptions.iterator();
